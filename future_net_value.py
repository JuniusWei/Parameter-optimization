...
#该函数用于计算夏普比率 返回只包含排名前30%的参数组 剔除了其余的列 用于第一次筛选
def cal_sharp_ratio(net_value_frame):
    profit_frame = net_value_frame.diff()
    #计算sharp ratio    #计算收益率frame
    sharp_ratio = (profit_frame.mean()/profit_frame.std()).to_frame()
    sharp_ratio.columns = ['sharp_ratio']
    #计算30%的参数组的长度
    new_length = round(len(net_value_frame.columns)*0.3)
    #按照sharp排序 并返回前30%
    top_sharp_name = sharp_ratio.sort_values(by='sharp_ratio',ascending=False).head(new_length).index
    #返回新的净值frame
    return net_value_frame[top_sharp_name]
...
def oneday_net_value(date):
    ...
    #先约束sharp位于前30% 剔除位于后70%的列
    day30_frame = cal_sharp_ratio(day30_frame)
    #计算30日的相关矩阵
